  ...
  <div>
    <div class="service-progress">
      <div :class="{
        ...
        <div class="tip-con" v-if="item.nodeStatus == 1">
          <div class="tip-title">{{item.nodeStatusName}}</div>
        </div>
        <div :class="{
            item: true,
          ...
          <el-popover placement="top" :title="item.statusName" width="200" trigger="hover" :content="item.nodeName?`${item.nodeName}`:''">
    ...
    <div class="service-desc">
      ...
      <div class="desc-btn" v-if="currData.buttonList&&currData.buttonList.length">
        <s-button type="primary" v-for="(o, i) in currData.buttonList" :key="i" :disabled="!!o.buttonDisabled" @click="flowButton(o)">
          {{o.buttonName}}
        </s-button>
      </div>
      <div v-if="demandData.annexFileShowUrl" class="pct100">
        <iframe  :src="demandData.annexFileShowUrl" frameborder="0" class="iframe"></iframe>
        <!-- <el-link  :href="demandData.annexFileShowUrl">{{demandData.annexFileName}}</el-link> -->
      </div>
      <div class="desc-explain">
        ...
        <div class="desc-explain-row" v-if="type == 'demand_stage'&&demandData.orderConsumeInfo&&demandData.orderConsumeInfo.length">
          <span>单品消耗 ：</span>
          <p>
            <el-table :data="demandData.orderConsumeInfo" style="width: 100%">
              <el-table-column prop="accountText" label="名称">
              </el-table-column>
              <el-table-column prop="consumeNum" label="消耗量">
              </el-table-column>
              <el-table-column prop="totalNum" label="总量">
              </el-table-column>
            </el-table>
          </p>
        </div>
        ...
        <div class="desc-explain-row exp-img" v-if="type == 'produce_stage'">
          <el-image v-if="designUrl" style="width: 400px; height: 220px" :src="designUrl" :preview-src-list="designList" lazy>
          </el-image>
        </div>
        <div class="desc-explain-row exp-decode" v-if="type == 'deliver_stage'">
          <div class="decode-col" v-for="(p, i) of previewList" :key="i">
            ...
            <el-popover placement="top" width="160" trigger="hover" v-if="p.type">
...
import getProgramme from '@/api/2543-get-service-order-interface-api-programme'
export default defineComponent({
  ...
  setup({ orderCode, list, type, index }, { emit }) {
    ...
    const initStage = () => {
      switch (type) {
        case 'demand_stage':
          ...
          break
        case 'programme_stage':
          getProgramme({ orderCode }).then(({ data }) => {
            set(progressData, 'demandData', data || {})
          })
          break
        case 'produce_stage':
          getDesign({ orderCode }).then(({ data }) => {
          ...
          break
        case 'deliver_stage':
          getPreviews({ orderCode }).then(({ data }) => {
            ...
            axios
              ...
              .then(({ data }) => {
                if (data.code == 'SYS0000') {
                  let len =
                    progressData.flowIndex < list.length
                      ? progressData.flowIndex + 1
                      : list.length
