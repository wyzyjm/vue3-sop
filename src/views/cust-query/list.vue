<template>
  <div>
    <div class="query-head mb20 flow-k">
      <span>客户名称：</span>
      <p>
        <el-autocomplete class="inline-input" v-model="form.custName" :fetch-suggestions="custSearch" placeholder="请输入客户名称" :trigger-on-focus="false" @select="custSelect">
        </el-autocomplete>
      </p>
        ...
        <el-tab-pane label="网站产品" name="first">
          <div class="cust-product p20">
            <div :class="{'cust-product-item':true, 'cust-product-active': item.displayUpdateBtn}" v-for="(item, index) of productList" :key="index">
              ...
              <div v-if="$hasPermissions('update_site')" class="cust-product-mark" @click="custProductEdit(item)"><i class="iconfont">&#xe615;</i></div>
            </div>
          </div>
        </el-tab-pane>
...
export default defineComponent({
  setup(props, { root }) {
    ...
    const custProductEdit = (item) => {
      serviceDialog.open({
        ...
        info: {
          ...
          domain:item.domain,
          operProductId:item.operProductId,
          ...custData.custInfo,
        },
        canList: [],
      })
    ...
    const __getCustInfo = () => {
      return getCustInfo({ custId: custData.custId }).then(({ data }) => {
        custData.info = data || {}
      })
    }

    const __getCustWebsite = () => {
      return getCustWebsite({ custId: custData.custId }).then(({ data }) => {
        custData.productList = data || []
      })
    }

    const __getCustTabs = (productId) => {
      return getCustTabs({ custId: custData.custId, productId }).then(
        ({ data }) => {
          let canArr = []
          let tabArr = data || []
    ...
    const __getOrderService = (serviceOrderId) => {
      getOrderService({ serviceOrderId }).then(({ data }) => {
